...
import { saveChat, updateChatMeta } from '@/app/actions';
...
async function submitUserMessage(content: string) {
  ...
  const result = await streamUI({
    ...
    messages: [
      ...aiState.get().chat.messages.map((message: any) => ({
        role: message.role,
...
export const AI = createAI<AIState, UIState>({
  ...
  onSetAIState: async ({ state }) => {
    ...
    const path = `/chat/${chatFromState.id}`

    const firstMessageContent = chatFromState.messages[0].content as string
    ...
    const chat: Chat = {
      ...
      messages: chatFromState.messages,
      meta: {
        title,
        path
      }
    ...
    if(chatFromState.meta.title === 'New chat') {
      // update the chat meta
      await updateChatMeta(chat.id, chat.meta)
    }
